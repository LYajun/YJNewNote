...
+ (void)showNewNoteViewOn:(UIView *)view newNoteType:(YJNewNoteType)newNoteType{
    YJNewNoteView *noteView = [[YJNewNoteView alloc] initWithFrame:CGRectMake(0, LG_ScreenHeight, LG_ScreenWidth, LG_ScreenHeight*0.9) newNoteType:newNoteType];
...
- (UILabel *)klgTextLab{
    if (!_klgTextLab) {
        ...
        NSString *text = [[YJNewNoteManager defaultManager].NoteTitle yj_deleteWhitespaceAndNewlineCharacter];
        if (text.length > 50) {
        ...
        NSMutableAttributedString *textAtt = text.yj_toMutableAttributedString;
