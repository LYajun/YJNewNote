...
- (void)layoutUI{
   ...
   [self.unBtn mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    self.klgTextView.hidden = IsStrEmpty([YJNewNoteManager defaultManager].Explain);
    if (!IsStrEmpty([YJNewNoteManager defaultManager].Explain)) {
        self.klgTextView.text = [YJNewNoteManager defaultManager].Explain;
    }
...
- (CGFloat)actualHeight{
    ...
    if (!IsStrEmpty([YJNewNoteManager defaultManager].Explain)) {
        textHeight = 10 + [[YJNewNoteManager defaultManager].Explain boundingRectWithSize:CGSizeMake([UIScreen mainScreen].bounds.size.width-40, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.klgTextView.font} context:nil].size.height+6;
        if (textHeight > 88) {
            textHeight = 88;
        }
    }
...
- (UITextView *)klgTextView{
    if (!_klgTextView) {
        ...
        _klgTextView.textColor = [UIColor yj_colorWithRed:37 green:37 blue:37];
        _klgTextView.font = [UIFont systemFontOfSize:16];
    }
    return _klgTextView;
}
- (UILabel *)titleLab{
...
- (YJNewNoteMarqueeLabel *)klgLab{
    if (!_klgLab) {
        ...
        _klgLab.textAlignment = NSTextAlignmentCenter;
    }
    return _klgLab;
}
@end
